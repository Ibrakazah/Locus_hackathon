#!/usr/bin/env bash
# Wrapper that runs the Octarin Antigravity capture hook (hook.mjs).
#
# Sources the committed team config (.octarin/project) then the per-user key
# (~/.octarin/octarin.env, written by `octarin login`), and exec's node on the
# single-file hook so Antigravity's stdin payload passes straight through.
# Fails open: any problem exits 0 so Antigravity is never blocked.
